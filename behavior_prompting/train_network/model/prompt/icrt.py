# modified version of icrt.py from ICRT repository
# notably we add support for action chunking
...
from icrt.models.policy.llama import Transformer, ModelArgs
from icrt.models.policy.action_head import MLPHead, GMMHead, DiffusionHead
...
class ICRT(nn.Module):
    ...
    - change default tensor type to torch.HalfTensor instead of torch.cuda.HalfTensor so that the model fully initializes on the CPU properly instead of partially on the GPU
    - add option end_of_prompt_token
    - additional method for forward action prediction on training batch
    - additional method for single step action prediction
    - add support for downsampling the prompt (action chunking)
    """
    ...
    def __init__(
        ...
        self.icrt_action_encoder = Mlp(in_features=self.action_dim*chunk_n_actions, out_features=self.latent_dim)
        ...
        torch.set_default_tensor_type(torch.HalfTensor) # MODIFIED from original
        ...
        self.chunk_n_actions = chunk_n_actions
    ...
    def state_dict(self, destination=None, prefix='', keep_vars=False):
        ...
        - fix bug where original implementation of this function would fail if prefix was not empty.
        """
        ...
        for k in trainable_params:
            new_state_dict[prefix + k] = state_dict[prefix + k] # the original implementation forgot to add the prefix so this function would error if called with a non-empty prefix
    ...
    def preprocessing(
        ...
        action : B, T, chunk_n_actions, action_dim (batch size, timesteps, chunk_n_actions, action_dim)
        ...
        # action processing
        B, T = action.shape[:2] # B, T, chunk_n_actions, action_dim
        action = action.reshape(B, T, self.chunk_n_actions * self.action_dim) # action chunks are put together along the action dimension
    ...
    def forward(self, sequences : dict) -> torch.Tensor:
        ...
        sequences: dict of tensors
            ...
            action : B, T, chunk_n_actions, num_pred_steps, action_dim (batch size, timesteps, chunk_n_actions, num_pred_steps, action_dim)
        ...
        # the inputs are only single step 
        proprio_in = proprio[:, :, 0] # B, T, num_pred_steps, proprio_dim -> B, T, proprio_dim
        action_in = action[:, :, :, 0] # B, T, chunk_n_actions, num_pred_steps, action_dim -> B, T, chunk_n_actions, action_dim; the input is the first action in the prediction horizon
        ...
        action_target = action[:, :, 0].reshape(B*T, self.action_dim * self.num_pred_steps) # B, T, chunk_n_actions, num_pred_steps, action_dim -> B, T, num_pred_steps, action_dim -> B*T, action_dim * num_pred_steps; the first action in the action chunk corresponds to the corresponding action prediction horizon that the state tokens predict
    ...
    def forward_inference(self, sequences : dict, start_pos: int, is_prompt: bool = False):
        """
        observations: B, T, N, 3, H, W (batch size, timesteps, num_cameras, 3, height, width)
        proprio: B, T, proprio_dim (batch size, timesteps, proprio_dim)
        action: B, T - 1, chunk_n_actions, action_dim (batch size, timesteps, chunk_n_actions, action_dim) # we don't have action for the last state
        (action: B, T, action_dim (batch size, timesteps, action_dim) # for state prediction)
        robot / dataset type (optional) : B, T, 1 (batch size, timesteps, 1)
        is_prompt: bool, whether the input is a prompt. If so then the last state and action of the sequence are replaced with the eos token
        ...
        if action is not None:
            action = action.reshape(B, action.shape[1], self.chunk_n_actions * self.action_dim) # B, T, chunk_n_actions, action_dim -> B, T, chunk_n_actions * action_dim
        ...
        if action_T == T - 1:
            if action is None: 
                action = torch.zeros(B, 1, self.action_dim*self.chunk_n_actions, dtype=proprio.dtype).to(device=proprio.device)
            else:
                action = torch.concatenate([action, torch.zeros(B, 1, self.action_dim*self.chunk_n_actions).to(device=action.device)], dim=1)
    ...
    def prompt(self, prompt_sequences : dict) -> None:
        ...
        Args:
            prompt_sequences: dict, a dictionary containing the sequences of observations and actions.
                observation : B, T, N, 3, H, W (batch size, timesteps, num_cameras, 3, height, width)
                proprio : B, T, num_pred_steps [optional], proprio_dim (batch size, timesteps, num_pred_steps, proprio_dim)
                action : B, T, chunk_n_actions, num_pred_steps [optional], action_dim (batch size, timesteps, chunk_n_actions, num_pred_steps, action_dim)

        Modifications from original ICRT:
        - support end of prompt token
        - fix bug in shape checking
        """
        prompt_sequences = prompt_sequences.copy()
        # the model takes one step proprio and action as input
        if len(prompt_sequences["action"].shape) == 5:
            prompt_sequences["action"] = prompt_sequences["action"][:, :, :, 0]
        if len(prompt_sequences["proprio"].shape) == 4:
        ...
        #     )
        self.forward_inference(prompt_sequences, self.start_pos, is_prompt=True)
    ...
    def get_action(self, observation : dict) -> torch.Tensor:
        """
        UPDATE FROM ORIGINAL: support batch actions. Removed action queue for now since we aren't using temporal weighting yet.
        Generating action from observations 

        Args:
            observation: dict, a dictionary containing the observations.
                "observation" : B, 1, N, 3, H, W or 1, 1, N, 3, H, W (batch size, T=1, num_cameras, 3, height, width)
                "proprio" : B, 1, proprio_dim or 1, 1, proprio_dim (batch size, T=1, proprio_dim)
        ...
        if self.last_observation is not None:
            for k in observation:
                observation[k] = torch.cat([self.last_observation[k][:, -1:], observation[k]], dim=1)
        self.last_observation = tmp
        observation["action"] = self.last_action

        action, eos_sequence = self.forward_inference(observation, self.start_pos)
        assert eos_sequence is None
        # action is (B, num_pred_steps, action_dim)
        B, num_pred_steps, action_dim = action.shape
        execed_action = action[:, :self.action_exec_horizon, :].clone() # (B, action_exec_horizon, action_dim)
        
        self.last_action = torch.cat([execed_action, torch.zeros((B, self.action_exec_horizon, 1), device=action.device)], dim=-1) # add eos (even though eos gets later removed since it's a deprecated feature...); last_action = (B, 1, action_dim+1)
        self.last_action = self.last_action.reshape(B, -1, self.chunk_n_actions, self.action_dim + 1) # (B, T, action_dim + 1) -> (B, T_down, chunk_n_actions, action_dim + 1); chunk action
        
        if self.first_obs:
            self.first_obs = False
        else:
            self.start_pos += 2 * self.action_exec_horizon // self.chunk_n_actions # we progress the start position by the number of steps we executed. We multiply by 2 because we account for states and actions which are separate inputs
    ...
    """Additional methods not in original ICRT"""
    def forward_action(self, sequences: dict) -> torch.Tensor:
        """
        Runs the forward pass on a training batch provided but returns the prediction actions instead of computing loss on those and actions and returning that as `forward` does
        sequences: dict of tensors
            observation : B, T, N, 3, H, W (batch size, timesteps, num_cameras, 3, height, width)
            proprio : B, T, num_pred_steps, proprio_dim (batch size, timesteps, proprio_dim)
            action : B, T, chunk_n_actions, num_pred_steps, action_dim (batch size, timesteps, chunk_n_actions, num_pred_steps, action_dim)
        return pred_action: B, T, num_pred_steps, action_dim
        """
        observation = sequences['observation']
        proprio = sequences['proprio']
        action = sequences['action']
        ...
        # preprocessing observations, proprio and actions
        f_sa = self.preprocessing(observation, proprio_in, action_in) # (B, T * (latent_len+1), self.latent_dim)
        ...
        out_a_decoded = self.icrt_action_decoder.pred(out_a).view(B, T, self.num_pred_steps, self.action_dim)
        return out_a_decoded
