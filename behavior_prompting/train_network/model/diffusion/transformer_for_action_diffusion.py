...
from behavior_prompting.train_network.model.common.transformer_decoder_with_attention import TransformerDecoderWithAttn, TransformerDecoderLayerWithAttn
...
class TransformerForActionDiffusion(ModuleAttrMixin):
    def __init__(self,
        ...
        max_cond_tokens = max_cond_tokens + 1 # +1 for time
    ...
    def forward(self, 
        ...
        **kwargs):
        ...
        cond_emb = torch.cat([time_emb, cond], dim=1)
        tc = cond_emb.shape[1]
        ...
        if memory_key_padding_mask is not None:
            B = memory_key_padding_mask.shape[0]
            memory_key_padding_mask = torch.cat([torch.zeros((B, 1), device=memory_key_padding_mask.device, dtype=torch.bool), memory_key_padding_mask], dim=1) # add a 0 to the beginning of the mask to indicate the first position (time) is valid

        # 3. process encoder
        if self.enable_encoder:
            cond_emb = self.encoder(cond_emb, src_key_padding_mask=memory_key_padding_mask)
