...
class VideoRecordingWrapper(gym.Wrapper):
    def __init__(self, 
        ...
        self.step_count = 0
        self.skip_next_video_reset_flag = False

    def reset(self, **kwargs):
        obs = super().reset(**kwargs)

        if self.skip_next_video_reset_flag:
            self.skip_next_video_reset_flag = False
        else:
            self.frames = list()
            self.step_count = 1
    ...
    def skip_next_video_reset(self):
        """Skip the next video reset. This means that we do not stop the video recoder, just call reset on the underlying environment."""
        self.skip_next_video_reset_flag = True
